...
function main() {
  ...
  local_s1="$( ciop-copy -o ${TMPDIR} ${online_resource} )"
  [[ -z ${local_s1} ]] && return ${ERR_NO_PRD} 

  # find MTD file in ${local_s1}
  s1mtd="$( find ${local_s1} -name "manifest.safe" )"

  [[ -z "${s1mtd}" ]] && return ${ERR_NO_S1MTD}

  out=${local_s1}_result
